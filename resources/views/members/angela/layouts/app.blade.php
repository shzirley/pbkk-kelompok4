...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('members/angela/angela.css') }}">
    ...
    <header class="sticky top-0 z-50 px-3 pt-3 md:px-6 md:pt-4" x-data="{ open: false }">
        <nav class="window-card mx-auto max-w-6xl">
            ...
            <div class="flex items-center justify-between gap-3 px-4 py-2.5 bg-brand-cream">
                <a href="{{ route('angela.home') }}" class="font-script text-2xl text-brand-pink-dark leading-none">
                    Angela Vania S.
                </a>

                {{-- Menu desktop --}}
                <div class="hidden md:flex items-center gap-5 text-sm font-semibold text-brand-frame">
                    <a href="{{ route('angela.home') }}" class="nav-link {{ request()->routeIs('home') ? 'is-active text-brand-pink-dark' : '' }}">Home</a>
                    <a href="{{ route('angela.projects') }}" class="nav-link {{ request()->routeIs('projects.*') ? 'is-active text-brand-pink-dark' : '' }}">Projects</a>
                    <a href="{{ route('angela.collection') }}" class="nav-link {{ request()->routeIs('collection') ? 'is-active text-brand-pink-dark' : '' }}">Collection</a>
                    <a href="{{ route('angela.contact') }}" class="nav-link {{ request()->routeIs('contact') ? 'is-active text-brand-pink-dark' : '' }}">Contact</a>
            ...
            <div x-show="open" x-transition x-cloak class="md:hidden flex flex-col gap-1 px-4 pb-3 bg-brand-cream text-sm font-semibold text-brand-frame border-t border-brand-frame/20">
                <a href="{{ route('angela.home') }}" class="py-2 {{ request()->routeIs('home') ? 'text-brand-pink-dark' : '' }}">Home</a>
                <a href="{{ route('angela.projects') }}" class="py-2 {{ request()->routeIs('projects.*') ? 'text-brand-pink-dark' : '' }}">Projects</a>
                <a href="{{ route('angela.collection') }}" class="py-2 {{ request()->routeIs('collection') ? 'text-brand-pink-dark' : '' }}">Collection</a>
                <a href="{{ route('angela.contact') }}" class="py-2 {{ request()->routeIs('contact') ? 'text-brand-pink-dark' : '' }}">Contact</a>
                <a href="{{ route('angela.resume') }}" class="py-2 font-bold text-brand-pink-dark">⬇ Download Resume</a>
            </div>
        </nav>
    </header>
