...
<section class="page-content">
    ...
    @foreach ($profile['projects'] as $item)
        <article class="project-card">
            <a class="project-preview preview-{{ $item['visual'] === 'calculator' ? 'numbers' : $item['visual'] }}" href="{{ route('kamal.'.$item['route']) }}{{ $item['anchor'] ?? '' }}" aria-label="Buka {{ $item['subtitle'] }}">
            ...
            <div class="card-links"><a href="{{ route('kamal.'.$item['route']) }}{{ $item['anchor'] ?? '' }}">Open project ↗</a><a href="{{ $profile['repository'] }}" target="_blank" rel="noopener noreferrer">Source code ↗</a></div>
        </article>
    @endforeach
    </div>
    <aside class="idea-banner"><div><p class="eyebrow">ON THE HORIZON</p><h2>What if an assistant could <em>act?</em></h2><p>Eksplorasi awal ide platform Agentic AI untuk proyek akhir PBKK.</p></div><a class="pill-button" href="{{ route('kamal.project') }}">Read the concept ↗</a></aside>
</section>
@endsection
