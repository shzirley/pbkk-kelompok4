...
<body class="bg-black text-white font-sans">
    ...
    <header id="site-header" class="fixed top-0 left-0 right-0 z-20 transition-colors duration-300 bg-transparent">
        <nav class="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-8 py-4 sm:py-6">
            <a href="{{ route('home') }}" class="font-serif text-base sm:text-xl font-semibold text-white">
                ITS Academic Profile
            </a>
            <div class="flex items-center gap-3 sm:gap-10 text-xs sm:text-sm">
                <a href="{{ route('home') }}" class="hover:text-gray-300 transition {{ request()->routeIs('home') ? 'text-white' : 'text-gray-300' }}">Home</a>
                <a href="{{ route('about') }}" class="hover:text-gray-300 transition {{ request()->routeIs('about') ? 'text-white' : 'text-gray-300' }}">About</a>
                <a href="{{ route('project') }}" class="hover:text-gray-300 transition {{ request()->routeIs('project') ? 'text-white' : 'text-gray-300' }}">Project</a>
                <a href="{{ route('calculator') }}" class="hover:text-gray-300 transition {{ request()->routeIs('calculator') ? 'text-white' : 'text-gray-300' }}">Calculator</a>
            </div>
        </nav>
    </header>
